  ...
  mcp.addTool({
    ...
    parameters: z.object({
      customer_id: z
        .string()
        .describe("Customer identifier. Ragen injects this; ignore it."),
      // Bounds in the schema, not only in the description: the description is
      // advice to the model and the schema is what rejects a bad call before
      // it reaches the upstream.
      latitude: z
        .number()
        .min(-90)
        .max(90)
        .describe("Degrees north, between -90 and 90."),
      longitude: z
        .number()
        .min(-180)
        .max(180)
        .describe("Degrees east, between -180 and 180."),
    }),
